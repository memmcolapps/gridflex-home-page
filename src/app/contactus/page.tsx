      ...
      <div className="flex flex-col px-8 md:px-0 gap-6 justify-center pt-20 py-10 items-center">
        ...
        <div className="text-center">
          <span className="text-3xl md:text-5xl font-light">
            Get in <span className="text-[var(--primary)]">touch with</span> us today
          </span>
        </div>
        <div className="flex text-sm md:text-normal flex-col md:w-2/3 font-extralight text-center">
          Have questions, feedback, or need assistance? Our team is here to help and support you every step of the way.
        </div>
        ...
        <div className="px-6 py-10 w-full md:px-10 md:py-10 md:w-1/2 mt-4 border border-gray-200 flex flex-col gap-8 rounded-lg">
          ...
          <div className="relative">
            <Label className="pb-2">Organization Size</Label>
            <button
              type="button"
              onClick={() => setOpen(!open)}
              className="w-full px-4 py-3 flex items-center justify-between border border-gray-200 rounded-lg bg-white"
            >
              <span className={`${selected ? 'text-black' : 'text-gray-200'} text-sm`}>
                {selected || "Select organization size"}
              </span>
              {open ? (
                <ChevronUp color="#BDBDBD" strokeWidth={0.75} />
              ) : (
                <ChevronDown color="#BDBDBD" strokeWidth={0.75} />
              )}
            </button>
                ...
                {items.map((item, idx) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      {item}
                      {selected === item && <Check color="#000000" strokeWidth={1} />}
                    </div>
                  </li>
                ))}
              </ul>
